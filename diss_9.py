...
def getCharmanderLink(soup):
    a_tags = soup.find_all('a', {'class': "ent-name"})
    for i in a_tags:
        if i.text == 'Charmander':
            return 'https://pokemondb.net' + i.get('href', None)
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_findLetters(self):
        self.assertEqual(findLetters(['Come eat lunch at 12', 'there"s a party @2pm', 'practice @7am', 'nothing']),
                         ['2pm', '7am'])
        self.assertEqual(findLetters(
            ['There is show @12pm if you want to join', 'I will be there @ 2pm', 'come at @3 pm will be better']),
            ['12pm', '3 pm'])
